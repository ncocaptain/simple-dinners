...
/**
 * Convert .jpg recipe images to .jpg and update project references.
 *
 * What it does:
 * 1) Recursively finds .jpg files in the target image directory
 * 2) Converts them to .jpg using sharp
 * 3) Recursively scans project source files and replaces ".jpg" with ".jpg"
 *
 * Notes:
...
async function convertWebpToJpg(webpPath) {
  const jpgPath = webpPath.replace(/\.jpg$/i, ".jpg");
...
async function updateReferences(files) {
  ...
  for (const file of files) {
    ...
    if (!original.includes(".jpg")) continue;

    const updated = original.replace(/\.jpg\b/g, ".jpg");
...
async function main() {
  ...
  console.log(`Delete .jpg:  ${DELETE_ORIGINALS ? "yes" : "no"}`);
  console.log("");
  ...
  const webpFiles = imageFiles.filter((file) => /\.jpg$/i.test(file));

  if (webpFiles.length === 0) {
    console.log("No .jpg files found.");
  } else {
    console.log(`Found ${webpFiles.length} .jpg files.\n`);
  }
  ...
  console.log("");
  console.log("Scanning project files for .jpg references...\n");
