...
export default function CookbookPage({ 
  ...
  const [newRecipe, setNewRecipe] = useState<Meal>({ 
    name: "", ingredients: "", instructions: "", photoUrl: "", effort: "normal"
  });
  ...
  const handleImport = async () => {
    if (!importUrl) return;
    toast("Magically importing recipe...");
    
    const scraped = await scrapeRecipe(importUrl);
    ...
    if (scraped) {
      ...
      toast("Couldn't find recipe data.");
    }
  };
          ...
          <section>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {lines.map((ing, index) => {
                ...
                return (
                  <div key={index} onClick={() => toggleIngredient(ing)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: "12px", background: isChecked ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.05)", border: isChecked ? "1px solid transparent" : "1px solid rgba(255,255,255,0.1)", cursor: "pointer", order: isChecked ? 100 : 0, opacity: isChecked ? 0.4 : 1, transition: "all 0.3s ease" }}>
