...
function slugify(value: string) {
  return value.trim().toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-");
}
...
export default function CookbookPage({ setMeals, cookbook, setCookbook, prefs }: { setMeals: React.Dispatch<React.SetStateAction<Record<string, Meal>>>; cookbook: CookbookEntry[]; setCookbook: React.Dispatch<React.SetStateAction<CookbookEntry[]>>; prefs: Preferences; }) {
  ...
  const [customEffort, setCustomEffort] = React.useState<"quick" | "normal" | "big">("quick");
  ...
  const normalizeEntry = React.useCallback((r: any): CookbookEntry => {
    ...
    return { ...r, id, slug: r?.slug ?? r?.id ?? id, favorite: Boolean(r?.favorite), createdAt: r?.createdAt ?? Date.now(), updatedAt: r?.updatedAt ?? Date.now(), name: r?.name ?? "Untitled", ingredients: r?.ingredients ?? "", instructions: r?.instructions ?? "", photoUrl: r?.photoUrl ?? "", tags: r?.tags ?? [], effort: r?.effort ?? "normal", notes: r?.notes ?? "" };
  }, []);
  ...
  const onImportRecipe = async () => {
    ...
    try {
      setIsImporting(true);
      const res = await fetch("/api/import-recipe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: importUrl }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Import failed");
      setCustomName(data.recipe.name || ""); setCustomIngredients(data.recipe.ingredients || ""); setCustomInstructions(data.recipe.instructions || ""); setCustomPhotoUrl(data.recipe.photoUrl || "");
      setShowImport(false); setShowAddRecipe(true); setImportUrl(""); toast("Imported!", "success");
    } catch (err: any) { toast(err.message, "error"); } finally { setIsImporting(false); }
  };
  ...
  const onAddCustomRecipe = () => {
    ...
    setCookbook(prev => { const next = [normalizeEntry(saved), ...prev]; persistCookbook(next as any); return next; });
                ...
                <div style={{ padding: 16, display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <label style={{ ...actionBtn }}>
                      <input type="file" accept="image/*" style={{ display: "none" }} onChange={async (e) => {
                        const file = e.target.files?.[0];
                        if(file) { setIsUploading(true); setEditDraft(prev => ({ ...prev, photoUrl: "" })); const url = await uploadImageToCloudinary(file); setEditDraft(prev => ({ ...prev, photoUrl: url })); setIsUploading(false); }
                      }} />
                ...
                <>
                  ...
                  <div style={{ padding: 16, display: "grid", gap: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      ...
                      <button style={actionBtn} onClick={async () => { await navigator.clipboard.writeText(`${PUBLIC_APP_URL}/recipe/${r.slug || r.id}`); toast("Copied!"); }}><Share2 size={14} /></button>
                      <button style={actionBtn} onClick={() => { setEditingId(rid); setEditDraft({...r}); }}><Pencil size={14} /></button>
                      <button style={{ ...actionBtn, background: "rgba(239,68,68,0.1)" }} onClick={() => { const next = cookbook.filter(x => x.id !== rid); setCookbook(next); persistCookbook(next as any); }}><Trash2 size={14} /></button>
                    </div>
